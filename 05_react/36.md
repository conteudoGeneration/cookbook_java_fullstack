...
O **Netlify** é uma plataforma de hospedagem voltada para aplicações **Frontend**, geralmente aquelas que não necessitam de banco de dados ou linguagens **back-end** para funcionar. Além da hospedagem, o Netlify oferece recursos adicionais, como **certificados SSL**, **CDN**, **autenticação**, **JWT** e até mesmo **formulários**, que podem ser gerenciados diretamente pela plataforma.

Trata-se de uma solução unificada que **automatiza o processo de deploy**, garantindo aplicações de **alta performance** e **fácil manutenção**. Um dos grandes diferenciais do Netlify é a integração com sistemas de versionamento: a partir de um **repositório Git remoto**, é possível configurar **implantações automáticas** da aplicação, simplificando todo o fluxo de desenvolvimento e entrega.

> Uma **CDN** (Content Delivery Network ou Rede de Entrega de Conteúdo) é um conjunto de servidores distribuídos geograficamente que acelera a entrega de conteúdos da web, aproximando-os fisicamente dos usuários finais.
...
- **Frontend**: fornece a interface gráfica de interação com o usuário e consome os dados processados pela API, normalmente hospedado em um servidor Node, como o **Netlify**, por exemplo.

O Netlify é uma **plataforma como serviço (PaaS)** que oferece uma experiência simplificada de **deploy contínuo** e **hospedagem** para aplicações modernas. Ele combina infraestrutura em nuvem com automação, permitindo que desenvolvedores foquem mais na aplicação e menos na configuração de servidores.

> **Modelos de Serviços na Nuvem:**
>
> - **Plataforma como Serviço (PaaS):** O provedor disponibiliza um ambiente em nuvem no qual os usuários podem **desenvolver, hospedar e entregar aplicativos** sem precisar gerenciar diretamente a infraestrutura. Além do **Netlify**, a **Vercel** e o **Microsoft Azure** também utilizam este modelo.
...
Um dos grandes diferenciais do Netlify é a oferta de **contas gratuitas**, que, apesar de apresentarem algumas limitações, permitem hospedar aplicações desenvolvidas em diversos **frameworks e bibliotecas frontend**, como **React, Angular, Vue, Next, entre outros**.

**Principais Limitações do Plano Gratuito**

- O plano gratuito não possui data de validade, pois é um nível permanente, e não uma avaliação temporária.
- Não é necessário informar cartão de crédito para realizar a inscrição.
- São disponibilizados **100 GB por mês de largura de banda**.
- O aplicativo não é finalizado ou reiniciado por inatividade, mesmo que permaneça sem receber requisições por um determinado período.

**Principais Vantagens do Plano Gratuito**

- **Hospedagem simplificada**: facilita a publicação de aplicações frontend sem necessidade de configurar servidores complexos.
- **Deploy automatizado via Git**: qualquer alteração feita no repositório remoto pode ser publicada automaticamente.
- **CDN integrada**: garante velocidade no carregamento, distribuindo os arquivos em servidores ao redor do mundo.
- **Certificado SSL gratuito**: possibilita que o site rode em HTTPS sem custos adicionais.
- **Autenticação e suporte a JWT**: oferece mecanismos de segurança prontos para proteger aplicações.
- **Gerenciamento de formulários**: permite capturar e organizar dados de formulários sem a necessidade de backend dedicado.
- **Alta performance**: otimiza a entrega de conteúdo, tornando as aplicações rápidas e escaláveis.
- **Fácil manutenção**: centraliza os recursos em uma única plataforma, reduzindo a complexidade da gestão.
...
1. De volta ao **Dashboard do Netlify**, clique no botão **Deploy with Github**, para listar os Repositórios do Github:
...
8. Para abrir a aplicação, clique no deploy concluído, como mostra a imagem abaixo:

<div align="center"><img src="https://i.imgur.com/ing0Gmy.png" title="source: imgur.com" /></div>

9. Na próxima tela, clique no link do Deploy, como mostra a figura abaixo:

<div align="center"><img src="https://i.imgur.com/a8kdp76.png" title="source: imgur.com" /></div>

10. Se tudo deu certo, será exibida a página de login do Projeto Integrador:

<div align="center"><img src="https://i.imgur.com/Nxz49XX.png" title="source: imgur.com" /></div>

11. Faça o login e teste toda a sua aplicação.
...
2. No item **Site Details**, clique no botão **Change Site Name**:
