...
**React Hooks** são funções especiais que permitem utilizar recursos fundamentais do React — como **Estado**, **Contexto** e **Ciclo de Vida** — dentro de **Componentes Funcionais**. Antes dos Hooks, esses recursos só estavam disponíveis em **Componentes de Classe**.

Introduzidos na versão 16.8 do React, os Hooks simplificaram a forma de escrever componentes e possibilitaram o compartilhamento de lógica sem a necessidade de herança ou estruturas complexas.

> ### Breve contexto histórico
>
> Nas primeiras versões do React, os componentes eram criados como **Classes**, que herdavam funcionalidades diretamente do React, como vemos no exemplo abaixo, de um componente contador: 
>
> ```tsx
> import React from "react";
> 
> export class ContadorClasse extends React.Component {
>   constructor(props) {
>     super(props);
> 
>     // Estado inicial
...
>     };
> 
>     // Bind manual para conseguir acessar 'this' dentro do método
...
> ```
>
> Antes dos Hooks, para usar **Estado** e atualizar valores, era necessário criar um componente baseado em classe e acessar recursos do React através de métodos internos.
>
> Observe no exemplo acima que:
>
> - O estado é definido dentro do **constructor**, usando `this.state`.
> - A atualização do estado é feita com **this.setState()**.
> - Métodos precisam de **bind** manual para acessar o `this` corretamente.
> - A estrutura é mais verbosa e exige mais código para fazer algo simples.
>
> Com a evolução da biblioteca, surgiram os **Componentes Funcionais**, que eram mais simples, porém limitados. Para solucionar essa limitação, os **Hooks** foram introduzidos, permitindo que funções comuns tivessem acesso a todos os recursos avançados da biblioteca.
>
> Essa mudança tornou os Componentes de Classe menos necessários, já que os Hooks oferecem maneiras mais simples, claras e modernas de trabalhar com estado e ciclo de vida.

<br />

O React possui vários Hooks nativos, entre eles:
...
Além dos Hooks nativos, muitas bibliotecas oferecem seus próprios Hooks e você também pode criar **Custom Hooks (Hooks Personalizados)** para encapsular lógicas reutilizáveis.

Os Hooks oferecem várias vantagens para o desenvolvimento em React:

1. **Reutilização de código:** permitem compartilhar lógica entre componentes, reduzindo duplicação.
2. **Legibilidade e simplicidade:** tornam os componentes mais diretos e fáceis de compreender.
3. **Flexibilidade:** permitem acessar funcionalidades avançadas (estado, contexto, ciclo de vida) em componentes funcionais.
4. **Facilidade de migração:** facilitam a modernização de projetos antigos que utilizam Classes.
5. **Melhor desempenho:** reduzem estruturas desnecessárias e evitam componentes extras no DOM.
...
A **Renderização Condicional** pode ser feita de diversas maneiras no **React**, entre elas:
