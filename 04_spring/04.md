...
<h2>👣 Passo 05 - Configurar o Fuso Horário da aplicação</h2>



Vamos configurar o fuso horário padrão da aplicação diretamente na classe principal (`BlogpessoalApplication`), para garantir que a JVM sempre trabalhe no horário de Brasília, independente do ambiente onde a aplicação for executada (local, nuvem, container, etc).

Isso é necessário porque muitos ambientes de nuvem e containers rodam com o fuso padrão em UTC, o que causa uma diferença de 3 horas entre o horário salvo no banco e o horário exibido pela aplicação — mesmo com o banco de dados configurado corretamente.

1. Abra a classe **BlogpessoalApplication**, localizada dentro do pacote principal da aplicação (**com.generation.blogpessoal**), dentro da  **Source Folder** principal (**src/main/java**)

<div align="center"><img src="https://imgur.com/sij7QK6.png" title="source: imgur.com" /></div>

2. Adicione a linha abaixo dentro do método **main**, antes da linha de inicialização da aplicação Spring

```java
TimeZone.setDefault(TimeZone.getTimeZone("America/Sao_Paulo"));
```

3. Abaixo você confere a classe **BlogpessoalApplication** com a alteração:

<div align="center"><img src="https://i.imgur.com/wx9WtuA.png" title="source: imgur.com" /></div>

Com o `TimeZone.setDefault(...)`, forçamos o fuso horário da JVM logo na inicialização, antes de qualquer conexão com o banco ser estabelecida. Assim, tanto em ambiente local (MySQL) quanto em produção (PostgreSQL na nuvem), os horários de criação e atualização das postagens (`@UpdateTimestamp`) são gravados e exibidos corretamente, sem depender do fuso configurado no sistema operacional ou no provedor de nuvem.

<br />

<h2>👣 Passo 06 - Configurar a Conexão com o Banco de dados</h2>
...
| **spring.jackson.time-zone**                             | Configura o fuso horário padrão utilizado pela dependência Jackson ao serializar o objeto no formato JSON. |
