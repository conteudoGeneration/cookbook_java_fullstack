...
**Optional (map):** É um contêiner para um valor que pode estar ausente. Em nosso contexto, tem como principal função evitar o erro do tipo **NullPointerException** (Objeto nulo), caso o Objeto Postagem procurado pelo Método findById(id) não seja encontrado. O método `findById(id)` retorna um `Optional<Postagem>`. Se a postagem existir, ela estará presente no `Optional`. Caso contrário, o `Optional` estará vazio — evitando o uso direto de `null` e possíveis `NullPointerException`.
