...
**Linha 29:** A anotação **`@JsonIgnoreProperties`** indica que determinadas propriedades do JSON serão ignoradas durante a serialização/desserialização, evitando problemas como **looping infinito** entre entidades relacionadas (assim como feito na Classe **Postagem**, aplicamos na Classe **Tema**).

Observe que, ao adicionar a propriedade **`allowSetters`** na anotação **`@JsonIgnoreProperties`**, o objeto **tema** foi incluído na propriedade **`value`**, diferentemente da Classe **Postagem**, onde apenas o objeto **postagem** era ignorado.

Ao utilizarmos a propriedade **`allowSetters = true`**, permitimos que os **métodos `set`** sejam executados normalmente, enquanto os **métodos `get`** são ignorados. Isso garante que o objeto **tema** seja considerado ao persistir ou atualizar dados, mas evita a desserialização de propriedades que poderiam causar ciclos infinitos, especialmente quando o tema possui postagens associadas.
