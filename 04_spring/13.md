...
<div align="left"><img src="https://i.imgur.com/Sd3yJ7p.png" title="source: imgur.com" /></div>

**Linha 32:** A anotação **`@OneToMany`** indica que a classe **`Tema`** está no lado **"um"** de um relacionamento **Um-para-Muitos (1:N)** com a classe **`Postagem`**. Isso significa que **um único objeto de `Tema` pode estar associado a várias instâncias de `Postagem`**.
...
**Linha 33:** A anotação **@JsonIgnoreProperties** indica que determinadas propriedades do objeto serão ignoradas durante a conversão entre objetos Java e JSON (Serialização e Deserialização do objeto), evitando problemas como o **loop infinito** entre entidades relacionadas.
...
**Linha 34:** Será criada uma Collection List contendo Objetos da Classe Postagem, que receberá todos os Objetos da  Classe Postagem associadas a cada Objeto da Classe Tema. 
