...
Linha 29: A anotação @JsonIgnoreProperties indica que determinadas propriedades do JSON serão ignoradas durante os processos de serialização ou desserialização, evitando problemas como loop infinito entre entidades relacionadas. (Segundo a documentação oficial do Jackson, essa anotação é usada em nível de classe para “suprimir” propriedades durante serialização e/ou ignorar propriedades desconhecidas durante deserialização.) 
Javadoc
+3
GitHub
+3
...
No caso em questão, assim como foi feito na classe Postagem, aplicamos @JsonIgnoreProperties também na classe Tema.

Note que, ao usar o atributo allowSetters dentro de @JsonIgnoreProperties, estamos configurando um comportamento especial: a propriedade tema foi incluída no value (ou seja, lista de nomes de propriedades a ignorar), mas com allowSetters = true. Em contraste, na classe Postagem, apenas a propriedade de postagem era ignorada (sem allowSetters).

Quando definimos allowSetters = true, permitimos que os métodos setters sejam considerados durante a desserialização (ou seja, Jackson pode atribuir valor à propriedade via setter), enquanto os métodos getters para aquela propriedade são ignorados para a serialização. Isso significa que:

Ao serializar (converter o objeto Java para JSON), a propriedade tema não será incluída no JSON — porque o getter será ignorado.

Ao desserializar (converter JSON em objeto Java), o Jackson ainda poderá invocar o setter para preencher a propriedade tema — evitando que a propriedade seja omitida completamente no momento de leitura do JSON.

Isso ajuda a prevenir ciclos infinitos (por exemplo, Tema contendo Postagem, que por sua vez referencia Tema) — pois evita que, na saída JSON, se inclua repetidamente as entidades relacionadas.

**Em resumo:**

value = { "tema" } indica que a propriedade tema está configurada para ser ignorada.

allowSetters = true assegura que apenas a leitura (getter/serialização) será ignorada, mas não a escrita (setter/desserialização).

Esse padrão é útil quando você quer incluir a referência da entidade relacionada durante persistência ou atualização, mas evitar que o JSON final continue expandindo o relacionamento indefinidamente.
