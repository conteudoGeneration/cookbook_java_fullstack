...
| Objeto      | Descrição                                                    | Exemplos práticos                                            |
| ----------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
...
| Método                | Descrição                                                    | Exemplos práticos                                            |
| --------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
...
Vamos ver na prática como acessar os elementos da DOM do nosso Projeto Guia **05-js-dom**:
...
<div align="center"><img src="https://imgur.com/5b23gfx.png" title="source: imgur.com" /></div>

Observe que os elementos selecionados serão exibidos no Console com toda a sua árvore de elementos, propriedades e atributos.
...
elemento_image.setAttribute('src', './assets/img/imagem_02.jpg');
```
...
1. O **DOM (Document Object Model)** representa a estrutura da página web como uma árvore de nós.
2. O JavaScript pode acessar e manipular o DOM por meio do objeto `document`.
3. O **BOM (Browser Object Model)** permite interagir com o navegador, como janelas, localização e histórico.
4. Métodos como `getElementById()` e `querySelector()` permitem selecionar elementos da página.
5. Propriedades como `innerHTML` e `textContent` permitem alterar o conteúdo dos elementos.
6. O uso de `textContent` é mais seguro do que `innerHTML`, reduzindo riscos de **XSS**.
7. É possível alterar estilos CSS diretamente via JavaScript usando `element.style`.
8. Elementos podem ser criados e removidos dinamicamente com `createElement()`, `appendChild()` e `remove()`.
9. A manipulação eficiente do DOM é essencial para desempenho e experiência do usuário.
10. Eventos e formulários são parte fundamental da manipulação da DOM em aplicações Frontend modernas.
11. Frameworks modernos abstraem a manipulação direta do DOM, mas compreender esses conceitos é fundamental.
