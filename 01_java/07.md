...
Todas essas três opções de Loops executam um conjunto de instruções repetidamente enquanto uma condição especificada permanecer verdadeira. 

Vale ressaltar que assim como nos Laços Condicionais, a aplicação de cada tipo de laço de repetição dependerá do contexto de utilização, e que essas estruturas também permitem inserir dentro dos seus blocos, a execução de:
...
4. O Laço de repetição **do-while** funciona de maneira semelhante ao loop while, com a diferença de que o código dentro do bloco sempre será executado pelo menos uma vez. A condição lógica é avaliada no final do bloco de código. Se a condição for verdadeira, o loop será executado repetidamente até que a condição seja falsa.
